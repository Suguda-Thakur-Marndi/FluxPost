...
function createProvider(type: ChannelTypeEnum, opts: { pkce?: boolean } = {}): OAuthProvider {
  return {
    ...
    getProfile: async ({ accessToken }) => {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch profile for ${type}: HTTP ${response.status}`);
      }
      const data = await response.json();
...
/**
 * Returns the real OAuth provider for the given channel type.
 * Returns null if the provider credentials are not configured — do NOT fall back to a mock.
 * Callers must check for null and return a 503 "provider not configured" response.
 */
export function getOAuthProvider(type: ChannelTypeEnum): OAuthProvider | null {
  const clientId = process.env[`${type}_CLIENT_ID`];
  if (!clientId) {
    return null;
  }
  return PROVIDERS[type] as OAuthProvider;
}
...
export async function refreshOauthToken(
  ...
  if (!provider) {
    throw new Error(`OAuth provider for ${type} is not configured.`);
  }
  if (!provider.refreshToken) {
    throw new Error(`Refresh token not supported for provider ${type}.`);
  }
